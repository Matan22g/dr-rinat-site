...
// .wrangler/tmp/bundle-BTjyFo/checked-fetch.js
...
// .wrangler/tmp/pages-UX24xP/functionsWorker-0.8978502005569496.mjs
...
__name2(forwardAudioToTelegram, "forwardAudioToTelegram");
...
__name2(forwardImageToTelegram, "forwardImageToTelegram");
...
__name2(getTelegramFile, "getTelegramFile");
...
__name2(uploadToWhatsApp, "uploadToWhatsApp");
  ...
  if (request.method === "POST") {
    try {
      ...
      if (msg) {
        ...
        const createNewTopic = /* @__PURE__ */ __name2(async () => {
...
\u{1F3F7}\uFE0F \u05DB\u05D5\u05EA\u05E8\u05EA: ${msg.referral.headline}` : "";
            const adSource = msg.referral.source_type ? ` (\u05DE\u05E7\u05D5\u05E8: ${msg.referral.source_type})` : "";
            const adId = msg.referral.source_id ? `
\u{1F194} \u05DE\u05D6\u05D4\u05D4: ${msg.referral.source_id}` : "";
            adInfo = `

\u{1F4E2} \u05D4\u05D2\u05D9\u05E2\u05D4 \u05DE\u05E7\u05DE\u05E4\u05D9\u05D9\u05DF \u05DE\u05DE\u05D5\u05DE\u05DF!${adSource}${adHeadline}${adId}`;
          }
          const backgroundTasks = [];
          if (isAudio) {
            backgroundTasks.push(forwardAudioToTelegram(msg.audio?.id || msg.voice?.id, session.threadId, `\u{1F464} \u05D4\u05E7\u05DC\u05D8\u05D4 \u05DE\u05D0\u05EA: ${currentName}${adInfo}`, disableNotification, env));
          } else if (isImage) {
            backgroundTasks.push(forwardImageToTelegram(msg.image.id, session.threadId, `\u{1F464} \u05DE\u05D0\u05EA: ${currentName}
\u{1F5BC}\uFE0F \u05EA\u05DE\u05D5\u05E0\u05D4
...
Phone: ${from}${adInfo}`,
          ...
          if (buttonId === "human") {
            ...
            backgroundTasks.push(
              env.SESSIONS_KV.put(from, JSON.stringify(session)),
              sendTelegram("editForumTopic", { message_thread_id: session.threadId, name: `\u{1F534} ${currentName} (${from.slice(-4)})` }, env)
            );
          } else if (buttonId === "main_booking") {
            backgroundTasks.push(sendTelegram("editForumTopic", { message_thread_id: session.threadId, name: `\u{1F534} ${currentName} (${from.slice(-4)})` }, env));
          }
          if ((!session.humanMode || justActivatedHuman) && nextStepId && BOT_FLOW[nextStepId]) {
            ...
            if (nextStepId !== "start" && buttons.length < 3 && !hasStartButton) {
              buttons.push({ id: "start", title: "\u05D7\u05D6\u05E8\u05D4 \u05DC\u05EA\u05E4\u05E8\u05D9\u05D8 \u{1F3E0}" });
            }
...
// .wrangler/tmp/bundle-BTjyFo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-BTjyFo/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.8978502005569496.js.map
